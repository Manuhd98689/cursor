...
class NGP_ProSony_URL_Viewer_Optimized {
    ...
    /**
     * Extract category from URL (assumes format like /category/subcategory/page)
     */
    private function extract_category_from_url($url) {
        ...
        return isset($path_parts[0]) ? $path_parts[0] : '';
    }
    
    /**
     * Get all URL-based categories for the dropdown
     */
    private function get_url_categories() {
        $cache_key = 'url_categories_list';
        ...
        // Get all posts to extract categories from their URLs
        $all_posts_result = $this->get_all_posts_for_categories();
        $categories = array();
        
        foreach ($all_posts_result as $item) {
            if (!empty($item['url'])) {
                $category = $this->extract_category_from_url($item['url']);
    ...
    /**
     * Get all posts for category extraction (optimized query)
     */
    private function get_all_posts_for_categories() {
        global $wpdb;
        
        $cache_key = 'all_posts_for_categories';
        ...
        // Get all published posts of relevant types
        $posts_query = "
            SELECT DISTINCT p.ID 
            FROM {$wpdb->posts} p 
            WHERE p.post_status = 'publish' 
            AND p.post_type IN ('page', 'pdf', 'partnerassets')
            ORDER BY p.ID DESC
            LIMIT 1000
        ";
        
        $post_results = $wpdb->get_results($posts_query);
        $all_posts = array();
        ...
        wp_cache_set($cache_key, $all_posts, $this->cache_group, $this->cache_expiry);
        return $all_posts;
    }
    ...
    private function get_filtered_posts_optimized($search = '', $selected_category = '', $limit = 20, $offset = 0) {
        ...
        // Get posts first, then filter by URL category
        $posts_query = "
        ...
        $post_results = $wpdb->get_results($wpdb->prepare($posts_query, $where_values));
        
        $filtered_posts = array();
        $all_matching_posts = array(); // For total count
        ...
        foreach ($post_results as $post_data) {
            ...
            if (!empty($selected_category)) {
                $url_category = $this->extract_category_from_url($url);
                if ($url_category !== $selected_category) {
                    continue;
                }
            }
            ...
            $all_matching_posts[] = $post_item;
        }
        
        // Apply pagination after filtering
        $total_count = count($all_matching_posts);
        $paged_posts = array_slice($all_matching_posts, $offset, $limit);
    ...
    private function render_search_form($search, $selected_category) {
        // Get URL-based categories instead of WordPress categories
        $categories = $this->get_url_categories();
        ...
        foreach ($categories as $category) {
            $selected = ($selected_category === $category) ? 'selected="selected"' : '';
            echo '<option value="' . esc_attr($category) . '" ' . $selected . '>';
